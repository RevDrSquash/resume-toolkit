...
Follow the procedure in `.claude/skills/resume-toolkit/skills/job-scout/SKILL.md` exactly. That skill is the source of truth for search flags, cache paths, shortlist format, ignore write-back, and degradation when OpenPostings is down.
...
- Write drops with `openpostings ignore <url>`. Never mark `applied` during scout.
