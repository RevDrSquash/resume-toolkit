...
**Judge against canonical sources, not prior artifacts.** Formatting is correct when it matches the bundled template (`.claude/skills/resume-toolkit/skills/build-targeted-resume/resume-template.html`) and `formatting-guide.md`; content is correct when it's supported by the `Work Experience/` notes. Never frame a fix as "make it match the previous resume" — prior resumes under `Job Applications/` are presumed stale (old layout, retired phrasing, outdated bullets, missing roles), so conforming toward one reintroduces the drift the build deliberately avoided. If the draft diverges from an older resume, that is usually correct, not a defect. See "Canonical sources & precedence" in `.claude/skills/resume-toolkit/reference/application-protocol.md`.
...
Coverage gaps are the most involved fixes — some are real candidate gaps that can't be edited away and may even change whether the user should apply at all — so settle what's in, out, or intentionally omitted before investing in prose. Derive the requirements from your review target: read the **JD directly** and identify the title, must-have skills/qualifications, preferred/nice-to-haves, implicit expectations, and required years; if you only have the signal report, use its structured fields (`Title to Mirror`, `Required Skills`, `Nice-to-Have Keywords`, `Action-Skill Pairs`, `Implicit Industry Expectations`) but treat them as lossy; if generic, judge against industry norms in `industry-signals.md`.

- [ ] **Title mirror.** Does the summary's opening sentence contain the exact target title (from the JD, or the signal report's `Title to Mirror`)? → Critical if absent.
